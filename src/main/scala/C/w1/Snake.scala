...
object SnakeFx extends JFXApp3:
  ...
  val initialSnake: Snake = List((250.0, 200.0), (225.0, 200.0), (200.0, 200.0))
  ...
  case class State(snake: Snake, food: Food):
    def check(newx: Double, newy: Double) =
      newx < 0 || newx >= 600 || newy < 0 || newy >= 600 ||
        snake.tail.contains((newx, newy))
    ...
    def newState(dir: Int): State =
      ...
      val newSnake: Snake =
        if check(newx, newy) then initialSnake
        else if (food == (newx, newy)) food :: snake
        else (newx, newy) :: snake.init

      val newFood = if food == (newx, newy) then randomFood else food
      State(newSnake, newFood)

    def rectangles: List[Rectangle] =
      square(food._1, food._2, Red) ::
        snake.map((x, y) => square(x, y, Green))
