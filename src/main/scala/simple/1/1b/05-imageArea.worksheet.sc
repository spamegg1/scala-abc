...
/** DESIGN a function called image-area that consumes an image and produces the area of
  * that image. For the area it is sufficient to just multiple the image's width by its
  * height. Follow the HtDF recipe and leave behind commented out versions of the stub and
  * template.
  */
