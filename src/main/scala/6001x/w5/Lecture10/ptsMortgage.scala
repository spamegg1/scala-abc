...
def findMaxPoints(
  ...
  for m <- 0 until totMonths
  do fixed1.makePayment
  ...
  for incr <- 0 until span
  do
    val pts    = minPts + incr * 0.1
    val fixed2 = FixedWithPts(amt, ptsRate, totMonths, pts)
    for m <- 0 until totMonths
    do fixed2.makePayment

    val ptsAmount = fixed2.getTotalPaid
    if ptsAmount - baseAmount > 0 then println(s"maximum points affordable ${pts}")
    pts
...
def findMaxPointsBisection(
  ...
  do
    ...
    if abs(ptsAmount - baseAmount) < eps then closeEnuf = true
    else if ptsAmount < baseAmount then high = pts
    else low = pts
