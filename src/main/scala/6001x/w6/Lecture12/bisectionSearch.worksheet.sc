def bisectSearch1(L: List[Int], e: Int): Boolean =
  ...
  else if L.length == 1 then L(0) == e
  else
    val half = L.length / 2
    if L(half) > e then bisectSearch1(L.take(half), e)
    else bisectSearch1(L.drop(half), e)
    ...
    if high == low then L(low) == e
      ...
      if L(mid) == e then true
        ...
        if low == mid then // nothing left to search
          false
        else bisectSearchHelper(L, e, low, mid - 1)
      else bisectSearchHelper(L, e, mid + 1, high)

  if L.length == 0 then false
  else bisectSearchHelper(L, e, 0, L.length - 1)
