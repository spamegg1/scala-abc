...
def getPermutations(sequence: String): List[String] =
  /** Enumerate all permutations of a given string sequence (string): an arbitrary string
    ...
    * part. Non-recursive solutions will not be accepted. Returns: a list of all
    * permutations of sequence Example: scala> getPermutations("abc") List("abc", "acb",
    * "bac", "bca", "cab", "cba") Note: depending on your implementation, you may return
    * the permutations in a different order than what is listed here.
    */
...
def ps4a =
  ...
  println(s"Expect Output: $exampleOutput")
  println(s"Actual Output: ${getPermutations(exampleInput)}")
