...
def savings(
  ...
  for month <- 0 until 36 do
    if month > 0 && month % 6 == 0 then salary += salary * semiRaise
...
def bisearchOneStep(
  ...
  if saving < downPay then newLeft = portion else newRight = portion
...
def bisearch(
  ...
  if math.abs(saving - downPay) < 100 then (portion, steps)
  else
