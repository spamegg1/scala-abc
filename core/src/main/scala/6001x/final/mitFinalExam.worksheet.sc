...
class MITCampus(centerLoc: Location, tentLoc: Location = Location(0, 0))
   ...
   * Assumes centerLoc and tentLoc are Location objects
   * Initializes a new Campus centered at location centerLoc
   * with a tent at location tentLoc
   */
  ...
  def addTent(newTentLoc: Location) =
    /**
     * Assumes newTentLoc is a Location
     * Adds newTentLoc to the campus only if the tent is
  ...
  def removeTent(tentLoc: Location) =
    /**
     * Assumes tentLoc is a Location
     * Removes tentLoc from the campus.
     * Raises a ValueError if there is not a tent at tentLoc.
